...
function generateNextSequentialID(sheetName, prefix) {
  const rows = globalCachedData[sheetName] || [];
  if (rows.length <= 1) {
    return prefix === 'P' ? 'P000001' : (prefix === 'G' ? 'G0001' : prefix + '0001');
  }
  ...
  for (let i = 1; i < rows.length; i++) {
    if (rows[i] && rows[i][0]) {
      ...
      if (idStr.startsWith(prefix)) {
        const numPartStr = idStr.substring(prefix.length);
        const num = parseInt(numPartStr, 10);
        if (!isNaN(num)) {
          if (num > maxNum) {
            maxNum = num;
            optimalLen = numPartStr.length;
          }
        }
      }
    }
  ...
  const paddedNumStr = nextNumStr.padStart(optimalLen || 4, '0');
...
function renderActiveScreensFromCache() {
  ...
  // 1. تشغيل دالة عرض الحوالات المعلقة فوراً لكافة المجموعات دون أي شروط
  loadRecorderPendingPayments();
  
  // 2. تحميل البيانات المعتمدة على اختيار المجموعة
...
function fillSelectorsFromCache() {
  ...
  ['admin-group-select', 'admin-report-group', 'recorder-group', 'recorder-report-group', 'payer-group', 'payer-report-group'].forEach(id => {
...
function loadCurrentContextData() {
  ...
  if(!activeGid && $('payer-report-group')) activeGid = $('payer-report-group').value;
  
  // إذا لم تكن هناك مجموعة مختارة، نكتفي بالخروج لأن الحوالات المعلقة تم تحميلها بالفعل في دالة الـ Render الرئيسة
  if(!activeGid) return;
  ...
  // تحديث وعرض المصاريف المنتظرة للموافقة لكافة المجموعات (لجهة الصرف Payer)
  const ulPayPending = $('payer-pending-list'); 
  if(ulPayPending) {
    ulPayPending.innerHTML = '';
    const rawExpenses = globalCachedData['Expenses'] || [];
    
    const pendingExps = convertRowsToObjects(rawExpenses).filter(e => {
      return String(e.Status).trim().toLowerCase() === 'pending' || String(e.Status).trim() === 'بانتظار الموافقة';
    });
    
    if(!pendingExps.length) { 
      ulPayPending.innerHTML = '<li class="muted">لا توجد مصاريف معلقة في أي مجموعة.</li>'; 
    } else {
      pendingExps.forEach(e => {
        const groupName = gMap[e.GroupID] || `مجموعة ${e.GroupID}`;
        const li = document.createElement('li'); li.className = 'summary-item';
        li.innerHTML = `<span style="color:#3b82f6; font-size:0.85rem; font-weight:bold;">[${groupName}]</span><br><strong>بقيمة: ${e.Amount} ج.م</strong> | البيان: ${e.Description}`;
        
        let sheetRowIndex = -1;
        for(let i = 1; i < rawExpenses.length; i++) { 
          if(rawExpenses[i] && String(rawExpenses[i][0]).trim() === String(e.ExpenseID).trim()) { 
            sheetRowIndex = i + 1; 
            break; 
          } 
        }
        ...
        const btnOk = document.createElement('button'); btnOk.id = `e-ok-${e.ExpenseID}`; btnOk.textContent = '✓ اعتماد المصروف'; btnOk.className = 'small';
        btnOk.onclick = () => handleBtnAction(`e-ok-${e.ExpenseID}`, '', async () => {
          if(sheetRowIndex > -1) { 
            await updateSheetCell('Expenses', `H${sheetRowIndex}`, 'Approved'); 
  ...
  if(tableBody) {
    ...
    if(!last10Exps.length) {
      tableBody.innerHTML = `<tr><td colspan="5" class="muted" style="text-align:center; padding:12px;">لا توجد مصاريف مسجلة لهذه المجموعة.</td></tr>`;
    } else {
...
// دالة فلترة وعرض الحوالات المنتظرة للاعتماد (مستقلة عن المجموعة النشطة ومحمية بالكامل ضد ترحل الأعمدة)
function loadRecorderPendingPayments() {
  const ul = $('recorder-payments-list'); if(!ul) return; ul.innerHTML = '';
  const rawPayments = globalCachedData['Payments'] || [];
  const groups = convertRowsToObjects(globalCachedData['Groups'] || []);
  const gMap = {}; groups.forEach(g => gMap[g.GroupID] = g.GroupName);
  
  // تصفية المعلق لجميع المجموعات بمعالجة ذكية في حال ترحلت كلمة "Pending" لعمود الملاحظات
  const pays = convertRowsToObjects(rawPayments).filter(p => {
    let actualStatus = String(p.Status).trim();
    const notesLower = String(p.Notes).trim().toLowerCase();
    
    if (!actualStatus && (notesLower === 'pending' || notesLower === 'approved' || notesLower === 'rejected')) {
      actualStatus = p.Notes;
    }
    
    const statusClean = actualStatus.toLowerCase();
    return statusClean === 'pending' || actualStatus === 'بانتظار تأكيد الاستلام';
  });
  
  const uMap = {}; convertRowsToObjects(globalCachedData['Users'] || []).forEach(u => uMap[u.UserID] = u.Name);

  if(!pays.length) { 
    ul.innerHTML = '<li class="muted">لا توجد حوالات بانتظار الاعتماد حالياً.</li>'; 
    return; 
  }
  
  pays.forEach(p => {
    const groupName = gMap[p.GroupID] || `مجموعة ${p.GroupID}`;
    // إخفاء كلمة "Pending" من خانة الملاحظات حتى لا تشوه شكل واجهة المستخدم
    const displayNotes = (String(p.Notes).trim().toLowerCase() === 'pending') ? 'بدون ملاحظات' : (p.Notes || 'بدون ملاحظات');
    
    const li = document.createElement('li'); li.className = 'summary-item';
    li.innerHTML = `<span style="color:#3b82f6; font-size:0.85rem; font-weight:bold;">[${groupName}]</span><br><strong>حوالة من: ${uMap[p.UserID_Payer] || p.UserID_Payer} بقيمة: ${p.Amount} ج.م</strong><br>البيان: ${displayNotes}`;
    
    let sheetRowIndex = -1;
    for(let i = 1; i < rawPayments.length; i++) { 
      if(rawPayments[i] && String(rawPayments[i][0]).trim() === String(p.PaymentID).trim()) { 
        sheetRowIndex = i + 1; 
        break; 
      } 
    }
    ...
    const btnApprove = document.createElement('button'); btnApprove.id = `p-app-${p.PaymentID}`; btnApprove.textContent = '✓ اعتماد الاستلام'; btnApprove.className = 'small';
    btnApprove.onclick = () => handleBtnAction(`p-app-${p.PaymentID}`, '', async () => {
      if(sheetRowIndex > -1) { 
        // عند الاعتماد، نكتب Approved في عمود الحالة (العمود الثامن H)
        // ومسح كلمة "Pending" المخزنة بالخطأ في عمود الملاحظات (العمود السابع G)
        if (String(p.Notes).trim().toLowerCase() === 'pending') {
          await updateSheetCell('Payments', `G${sheetRowIndex}`, ''); 
        }
        await updateSheetCell('Payments', `H${sheetRowIndex}`, 'Approved'); 
        await refreshAllSheetsCacheInBackground(); 
      }
    }, () => { li.remove(); });
    
    const btnReject = document.createElement('button'); btnReject.id = `p-rej-${p.PaymentID}`; btnReject.textContent = '❌ رفض الحوالة'; btnReject.className = 'small'; btnReject.style.backgroundColor = '#fb7185';
    btnReject.onclick = () => handleBtnAction(`p-rej-${p.PaymentID}`, '', async () => {
      if(sheetRowIndex > -1) {
        if (String(p.Notes).trim().toLowerCase() === 'pending') {
          await updateSheetCell('Payments', `G${sheetRowIndex}`, ''); 
        }
        await updateSheetCell('Payments', `H${sheetRowIndex}`, 'Rejected'); 
        await refreshAllSheetsCacheInBackground(); 
      }
    }, () => { li.remove(); });
...
function runUnifiedReport(groupSelectId, typeSelectId, fromDateId, toDateId, outputDivId, buttonId) {
  ...
  handleBtnAction(buttonId, '', async () => {
    ...
    const expenses = convertRowsToObjects(globalCachedData['Expenses'] || []).filter(e => String(e.GroupID) === String(gid) && String(e.Status).trim().toLowerCase() === 'approved');
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  if($('payer-report-group')) $('payer-report-group').onchange = loadCurrentContextData;
  ...
  $('recorder-report-btn').onclick = () => runUnifiedReport('recorder-report-group', 'recorder-report-type', 'recorder-report-from', 'recorder-report-to', 'recorder-report-result', 'recorder-report-btn');
  $('payer-report-btn').onclick = () => runUnifiedReport('payer-report-group', 'payer-report-type', 'payer-report-from', 'payer-report-to', 'payer-report-result', 'payer-report-btn');
  ...
  $('save-group-btn').onclick = async () => {
    ...
    if(!gid) gid = generateNextSequentialID('Groups', 'G');
    
    handleBtnAction('save-group-btn', '', async () => {
      const rawGroups = globalCachedData['Groups'] || [];
      let foundGIdx = -1;
      for(let i=1; i<rawGroups.length; i++) { if(rawGroups[i] && String(rawGroups[i][0]).trim() === String(gid).trim()) { foundGIdx = i; break; } }
      if(foundGIdx > -1) {
      ...
      const catRows = [];
      const splitLines = gCatsStr.split('\n');
      let indexCounter = 1;
      splitLines.forEach(line => {
        const name = line.trim(); if(!name) return;
        const nextCatId = generateNextSequentialID('ExpenseCategories', 'C');
        const finalNum = parseInt(nextCatId.replace('C', ''), 10) + (indexCounter - 1);
        const paddedId = 'C' + String(finalNum).padStart(4, '0');
        catRows.push([paddedId, gid, name, 'TRUE']);
        indexCounter++;
      });
      ...
      setText('admin-group-msg', 'تم حفظ المجموعة والتصنيفات بنجاح');
    });
  };
  ...
  $('save-user-btn').onclick = async () => {
    ...
    const uId = generateNextSequentialID('Users', 'U');
    ...
    handleBtnAction('save-user-btn', '', async () => {
      ...
      setText('admin-user-msg', 'تم حفظ الحساب بنجاح');
    });
  };
  ...
  $('recorder-save-expense').onclick = async () => {
    ...
    const expId = generateNextSequentialID('Expenses', 'E');
  ...
  $('payer-save-payment').onclick = async () => {
    ...
    const pId = generateNextSequentialID('Payments', 'P');
    ...
    handleBtnAction('payer-save-payment', '', async () => {
      ...
      setText('payer-pay-msg', 'تم حفظ معاملة السداد وبانتظار تأكيد الاستلام');
      loadCurrentContextData();
    });
  };
